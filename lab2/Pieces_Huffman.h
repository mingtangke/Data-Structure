...
public:
    ...
    std::vector<std::string> proc_buffer;
    ...
    void Display_File_With_Huffman (std::string address){
        for(int i = 0;i < total_word; i++){
            for(int j = 1;j <= leaf_num;j++){
                if(Diction[j].data == proc_buffer[i]){
    ...
    void Generate_Huffman_Code() {
        ...
        Create_Huffman();
        Encode_Huffman();
        Display_Huffman_Code();
        Display_File_With_Huffman("E:\\HomeWork\\lab2\\output\\Pieces_File_With_Huffman.txt");
        //Read_Huffman_File("Pieces_File_With_Huffman.txt");
        Save_Bin_File();
        Read_Bin_File();
        Decode();
        conclusion();
...
virtual inline void Read_Bin_File(){
        std::ifstream readFile("Pieces_Compact_File.bin",std::ios::binary);
        if(!readFile.is_open()){
            std::cerr<<"Fail to open Pieces_Compact_File.bin"<<std::endl;
        }
    ...
    virtual inline void Save_Bin_File(){
        ...
        std::ofstream outputFile("Pieces_Compact_File.bin", std::ios::binary);
        if (!outputFile.is_open()) {
        throw std::logic_error("Failed to open Pieces_Compact_File.bin");
        }
    ...
    inline void conclusion(){
        int add = 8 - decode_buffer.size() % 8;
        double rate = 1- (decode_buffer.size() + add + 8)/(8.0*buffer.size());
        std::cout<<"compression ratio :"<<rate<<std::endl;
    }
    inline void Display_Huffman_Code() {
        std::ofstream outFile("Pieces_Compression_Dictionary.txt");
        if (outFile.is_open()) {
            outFile << "Element  Huffman_Code Frequency " << std::endl;
            for (int i = 1; i <= leaf_num; i++) {
                outFile << Diction[i].data << "       " << Diction[i].HC << "     " << 1.0 * Diction[i].weight / total << std::endl;
            }
            outFile.close();
    ...
    inline void Caculate_Weight(){
        std::string temp = "";
        for(int i = 0;i < static_cast<int>(buffer.size());){
            //int k = 0;
            bool word = false;
            if(std::isalpha(buffer[i]) || i+1<static_cast<int>(buffer.size())&&
            !std::isalpha(buffer[i])&&std::isalpha(buffer[i+1]) && !isspace(buffer[i])){
            ...
            if(word) continue;
            //std::cout<<temp;
            proc_buffer.push_back(temp);
            proc_buffer.push_back(std::string(1, buffer[i]));//bug
            temp.clear();
            i++;
        }

        Diction.push_back({-1,"&",""});
        for(size_t i = 0; i< proc_buffer.size();i++){
            bool find = false;
            for(size_t k = 0; k<Diction.size(); k++){
                if(Diction[k].data == proc_buffer[i]){
                    Diction[k].weight++;
                    find = true;
                    break;
                }
            }
            if(!find) Diction.push_back({1,proc_buffer[i],""});
        }
        // for(size_t i = 0; i<Diction.size() ;i++){
        //     std::cout<<Diction[i].data<<"====="<<Diction[i].weight<<std::endl;
        // }
        leaf_num = static_cast<int>(Diction.size()) - 1;
        node_num = 2 * leaf_num - 1;
        total_word = proc_buffer.size();
    }
    ...
    inline void Select_Two(int &min1, int &min2, int upper) {
